...
for epoch in range(1000):
    ...
    for i in range(dataset.my_dataset_train.number_of_batch):
        ...
        if batch_name == 'pp': # Outdated, but it should always be pp here.
                ...
                if batch_name == 'pp':
                    ...
                    while np.random.uniform() < 0.5 and ts_cnt < 4:
                        ...
                        x_to_give['objects_state'][:, 1:,
                                                :6] = y_pred[:, ts_cnt-1, :, :]
                        y_pred[:, ts_cnt, :, :] = dataset.gp_pp(x_to_give)
                    metric_loss = dataset.gp_pp.loss(dataset.scaler.transform_vel(y['objects_state_next'][:, :ts_cnt+1, :, 3:6]),
                                         dataset.scaler.transform_vel(y_pred[:, :ts_cnt+1, :, 3:6]))
                    losses_val_PP.append(metric_loss.item())
                    del x, y, y_pred
    ...
    plt.savefig('articulated_pp.png')
    plt.close(f)
    torch.save(Net.state_dict(), 'articulated_pp.pt')
